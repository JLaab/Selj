...
const normalizeFilters = (raw: unknown): { filters: FilterOption[]; errors: string[] } => {
  const errors: string[] = [];
  ...
  raw.forEach((item, idx) => {
    ...
    if (item?.type === "select" || item?.type === "chip") {
      const options = Array.isArray(item.options)
        ...
        : [];
      if (!label) errors.push(`Filter ${idx + 1}: label saknas`);
      if (options.length === 0) errors.push(`Filter ${label || idx + 1}: lägg till minst ett val`);
    ...
    if (item?.type === "range") {
      ...
      if (!label) errors.push(`Filter ${idx + 1}: label saknas`);
      if (!min || !max) errors.push(`Filter ${label || idx + 1}: min och max krävs`);
      if (label && min && max) {
    ...
    if (item?.type) {
      errors.push(`Filter ${idx + 1}: okänd typ ${item.type}`);
    }
  });
...
export async function POST(request: Request) {
  ...
  const { filters, errors: filterErrors } = normalizeFilters(body.filters ?? []);
  ...
  const newCat: Category = { value, label, filters, parentValue };
...
export async function PUT(request: Request) {
  ...
  if (body.filters !== undefined) {
    const { filters: normalized, errors } = normalizeFilters(body.filters);
  ...
  const label =
    typeof body.label === "string" && body.label.trim().length > 0
      ? body.label.trim()
      : existing.label;
  const updated = db.updateCategory(value, { label, filters, parentValue });
